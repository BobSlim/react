const decimalize = (number = 0, places = 2) => {
	...
	const addedZeroes =
		places + 1 - Math.min(3, number.slice(decimalPlace).length);
	return number + "0".repeat(addedZeroes);
};

export const displayPrice = (price = 0) =>
	(price < 0 ? "-$" : "$") + decimalize(Math.abs(price));
...
export const findObjectById = (array, id) => array[findIndexById(array, id)];
